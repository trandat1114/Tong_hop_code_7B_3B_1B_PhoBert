...
─────────────────────────────────────────────────────
PhoBERT         0.7936    0.5991         7.0    0.135B  ← Best F1 + Speed
LLM 1.5B (LoRA) 0.7280    0.5103         3.7    1.54B
LLM 3B  (LoRA)  0.6990    0.5361        11.0    3.09B
LLM 7B  (LoRA)  0.6458    0.4244         1.6    7.62B
```
...
![Hình 1: Phân phối nhãn ViHSD — mất cân bằng lớp rõ rệt (CLEAN chiếm đa số)](figures/01_data_distribution.png)

*Hình 1: Phân phối nhãn ViHSD — mất cân bằng lớp rõ rệt (CLEAN chiếm đa số)*
...
|-----------------|-----------------|-----------------|----------|--------|---------|-------------|--------|-------|
| **PhoBERT**     | **0.7936**      | **0.5991**      | 0.8876 | 0.3641 | **0.5456** | **7.0** | 0.135B | 92min |
| LLM 1.5B (LoRA) | 0.7280          | 0.5103          | 0.8572 | 0.3053 | 0.3684 | 3.7 | 1.54B | 85min |
| LLM 3B (LoRA)   | 0.6990          | 0.5361          | 0.8313 | 0.3038 | 0.4732 | 11.0 | 3.09B | 86min |
| LLM 7B (LoRA)   | 0.6458          | 0.4244          | 0.8103 | 0.2623 | 0.2005 | 1.6 | 7.62B | 85min |
...
![Hình 2: F1-Macro tổng thể (trái) và F1 từng lớp (phải) cho 4 models fine-tuned](figures/04_f1_comparison.png)

*Hình 2: F1-Macro tổng thể (trái) và F1 từng lớp (phải) cho 4 models fine-tuned*


**Nhận xét:**
- **PhoBERT** đạt F1-Macro cao nhất (0.5991)
- Khoảng cách PhoBERT vs LLM 7B: chỉ 0.1747 — rất gần nhau dù kích thước chênh 56×
...
![Hình 3: Confusion Matrix chuẩn hóa cho 4 models — màu đậm = tỷ lệ phân loại đúng cao](figures/05_confusion_matrices.png)

*Hình 3: Confusion Matrix chuẩn hóa cho 4 models — màu đậm = tỷ lệ phân loại đúng cao*


**Phân tích từng model:**
- **PhoBERT:** Phân loại cân bằng nhất, HATE recall tốt nhất (0.5456)
- **LLM 7B:** Tốt ở OFFENSIVE (0.2623), có thể do contextual understanding tốt hơn
- **LLM 3B:** Hiệu suất trung gian giữa 7B và 1.5B
- **LLM 1.5B:** Capacity thấp nhất → F1-HATE thấp nhất (0.3684)
...
![Hình 4: Tốc độ inference (log scale) và trade-off kích thước vs F1](figures/06_speed_size.png)

*Hình 4: Tốc độ inference (log scale) và trade-off kích thước vs F1*


| So sánh               | Tỷ lệ tốc độ          |
|-----------------------|-----------------------|
| PhoBERT vs LLM 7B     | **4×** nhanh hơn      |
| PhoBERT vs LLM 3B     | **1×** nhanh hơn      |
| PhoBERT vs LLM 1.5B   | **2×** nhanh hơn      |
| LLM 1.5B vs LLM 7B    | **2×** nhanh hơn      |
...
![Hình 5: Radar chart 6 chiều — diện tích lớn hơn = tốt hơn toàn diện](figures/07_radar.png)

*Hình 5: Radar chart 6 chiều — diện tích lớn hơn = tốt hơn toàn diện*
...
![Hình 6: Trade-off giữa tốc độ và F1. Góc phải trên = vùng lý tưởng.](figures/08_tradeoff.png)

*Hình 6: Trade-off giữa tốc độ và F1. Góc phải trên = vùng lý tưởng.*


**Góc phải trên** (F1 cao, tốc độ cao) = vùng lý tưởng:
- **PhoBERT** chiếm vị trí tối ưu: F1=0.5991, Speed=7.0sps
- LLM 7B: F1 gần bằng (0.4244) nhưng tốc độ kém 4×

### 5.6 Thời gian Training

![Hình 7: Thời gian training thực tế so với giới hạn 90 phút](figures/09_training_time.png)

*Hình 7: Thời gian training thực tế so với giới hạn 90 phút*
...
![Hình 8: Bảng so sánh tổng hợp với highlight model tốt nhất (nền xanh)](figures/10_summary_table.png)

*Hình 8: Bảng so sánh tổng hợp với highlight model tốt nhất (nền xanh)*
...
| **LLM 7B** | F1 cao, hiểu ngữ cảnh tốt, có thể sinh explanation | Chậm nhất (1.6sps), VRAM cao |
...
Bước 1: Thu thập dữ liệu chưa có nhãn (Facebook, TikTok, YouTube...)
         ↓
Bước 2: PhoBERT inference (7 sps)
...
|---------|-------|-------|
| **Production labeling** | **PhoBERT** | F1 cao nhất + 7sps |
...
- **PhoBERT** (F1=0.5991) là model tốt nhất
- LLM 7B đạt F1=0.4244 — gần bằng PhoBERT nhưng chậm hơn 4× và phức tạp hơn nhiều
...
| Use case | Đề xuất |
|---------|---------|
| API production (>100 QPS) | PhoBERT — 7sps, F1=0.5991 |
| Nghiên cứu, cần explanation | LLM 7B — F1=0.4244, có thể sinh lý do |
| Cân bằng tốt | LLM 3B — F1=0.5361, 11sps |
| Thiết bị giới hạn (<4GB VRAM) | LLM 1.5B — 1.54B, 4sps |
...
| **Nghiên cứu này** | **PhoBERT-base-v2** | **0.5991** | **2026** |
| **Nghiên cứu này** | **LLM 7B (LoRA)** | **0.4244** | **2026** |
| **Nghiên cứu này** | **LLM 3B (LoRA)** | **0.5361** | **2026** |
| **Nghiên cứu này** | **LLM 1.5B (LoRA)** | **0.5103** | **2026** |
...
*Báo cáo sinh tự động bởi `generate_report.py` — 17/06/2026 02:31*
